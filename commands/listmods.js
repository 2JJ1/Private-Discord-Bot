...
module.exports = {
	...
	async execute(interaction){
		...
		let moderators = (await interaction.guild.members.fetch()).filter(member => member.roles.cache.find(roles => roles.name.toLowerCase() === 'moderator') && member.user.bot === false)
		.map(member =>  `<@${member.user.id}>`) //Converts to mentions
		if(moderators.length > 0) list += "\r\nModerators: " + moderators.join(", ")

		//List people with the mini-moderator role (If any)
		let miniModerators = (await interaction.guild.members.fetch()).filter(member => member.roles.cache.find(roles => roles.name.toLowerCase() === 'mini-moderator') && member.user.bot === false)
		.map(member =>  `<@${member.user.id}>`) //Converts to mentions
